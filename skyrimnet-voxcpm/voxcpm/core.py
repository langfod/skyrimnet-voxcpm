...
#
    # ------------------------------------------------------------------ #
    # Tensor-optimized generation (stays on CUDA)
    # ------------------------------------------------------------------ #
    def _generate_with_prompt_cache_tensor(self,
        ...
        """Generate speech using pre-built prompt cache, returning GPU tensors.
        
        This is a performance-optimized version that keeps tensors on CUDA
        throughout the pipeline, avoiding unnecessary CPU transfers.
        
        Args:
            text: Input text to synthesize.
            prompt_cache: Pre-built prompt cache from build_prompt_cache().
            cfg_value: Guidance scale for the generation model.
            inference_timesteps: Number of inference steps.
        ...
        Returns:
            Generator of torch.Tensor: Waveform tensors on GPU (or model device).
            Yields audio chunks if streaming=True, otherwise yields single final tensor.
        """
        if not text.strip() or not isinstance(text, str):
            raise ValueError("target text must be a non-empty string")
        ...
        # Yield GPU tensors directly - no .cpu() conversion
        for wav, _, _ in generate_result:
            yield wav  # Keep on GPU
