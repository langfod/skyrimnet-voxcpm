...
binaries = collect_dynamic_libs('triton-windows')
...
module_collection_mode = {
    'triton-windows': 'pyz+py',
    'triton-windows.runtime.jit': 'py',
    'triton-windows.compiler': 'py',
    'triton-windows.language': 'py',
}

# triton 3.0.0 introduced `triton.backends` sub-package with backend-specific files.
if is_module_satisfies('triton-windows >= 3.0.0'):
    # Collect backend sub-modules/packages.
    hiddenimports += collect_submodules('triton-windows.backends')
    ...
    hiddenimports += collect_submodules('triton-windows.backends.amd')

    # Collect ptxas compiler files from `triton/backends/nvidia`, and the HIP/ROCm files from `triton/backends/amd`.
    datas += collect_data_files('triton-windows.backends')
else:
    ...
    datas += collect_data_files('triton-windows.third_party.cuda')